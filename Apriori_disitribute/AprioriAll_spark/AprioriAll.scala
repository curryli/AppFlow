...
object Apriori {
  ...
  def AprioriGen(L_k: Array[List[String]]): Array[(List[String])] = {
    ...
    for(i<-0 to len_Lk){
      for(j<-(i+1) to len_Lk){
		L1_ini = L_k(i).init   //   L_k(i)[:len_Lk-1]
	    L1_tail = L_k(i).tail  //   L_k(i)[1:]
		L2_ini = L_k(j).init


		    if(L1_ini == L2_ini){
              val c1 = L1_ini + L_k(i).last + L_k(j).last
              if(!has_infrequent_subset(c1,L_k)){   //判断刚生成的K+1候选集c 
                Ck += c1
              }
			  
			  val c2 = L1_ini + L_k(j).last + L_k(i).last 
              if(!has_infrequent_subset(c2,L_k)){   //判断刚生成的K+1候选集c 
                Ck += c2
              }
            }
		  
		    if(L1_tail == L2_ini){
              val c = L_k(i).first + L1_tail + L_k(j).last
